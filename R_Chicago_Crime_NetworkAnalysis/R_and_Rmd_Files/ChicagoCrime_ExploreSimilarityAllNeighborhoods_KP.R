######################################################################################################
# 
# ChicagoCrime_ExploreSimilarityAllNeighborhoods.R
# 
# The file performs analysis on the similarity neighborhood projections of all crimes 
# from the Feb 2017 Chicago Crime datasets.  The first step is to 
# create the similarity projectionof the graph data.  
...
all_gr = read.graph("feb_all_neighborhoods_similarity.graphml", format = "graphml")
...
all_clean = delete_vertex_attr(all_gr, "Crime.Type")
...
e_perc_rem = 0.90
e_edge_w = E(all_clean)$weight
e_sorted_edge_w = sort(e_edge_w)
e_thresh = quantile(e_sorted_edge_w, e_perc_rem)
...
all_filter = delete.edges(all_filter, which(E(all_filter)$weight == 1))
...
g_combined_log = ggplot(log_weight_df, aes(x=wdeg, y=tabulate.wdeg_bins.)) +
  geom_point() + geom_line() +
...
v_comp_sizes = sapply(all_decomp, vcount)
v_giant_ndx = which(v_comp_sizes == max(v_comp_sizes))
v_giant = all_decomp[[v_giant_ndx]]
...
# Save updated files
out_file_all = paste(path, "feb_all_neighborhoods_90_similarity_filtered.graphml", sep = "")
